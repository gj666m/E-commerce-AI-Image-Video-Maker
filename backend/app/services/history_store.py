...
async def clear_all_history_admin(username: str | None = None) -> int:
    """管理员硬删全部历史（真删：删盘文件 + DELETE 整行，不可恢复）
    ...
    try:
        if username:
            ...
            logger.info(f"管理员硬删所有用户历史，共 {count} 条")
        return count
    finally:
        await db.close()
