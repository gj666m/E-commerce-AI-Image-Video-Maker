...
# === 视频提示词反推系统提示词（Sora 结构化分镜格式） ===
REVERSE_VIDEO_PROMPT_SYSTEM = """你是一个专业的 AI 视频提示词工程师，擅长分析视频并反推出可以直接用于 Sora / Seedance / RunwayML / Pika 等 AI 视频生成模型的高质量提示词。
...
class GeminiProvider:
    ...
    async def reverse_video_prompt(
        ...
        Args:
            ...
            style: 输出风格，目前支持 sora_structured（其他值降级到默认）
            extra_prompt: 用户额外要求（如"更简洁" / "英文输出"）
        Returns:
            Sora 结构化分镜 prompt 文本（中文，含英文关键词）
        """
        ...
        # 构造文本指令
        text = "请分析这段视频，反推出 Sora 结构化分镜格式的 AI 视频生成 prompt。"
